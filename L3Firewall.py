...
class Firewall (EventMixin):
	...
	def __init__ (self,l2config,l3config):
		...
		# Marco: make a set to prevent duplicates
		self.disbaled_MAC_pair = set() # Shore a tuple of MAC pair which will be installed into the flow table of each switch.
		self.fwconfig = list()
		# Marco: Maps src MAC to src IP
		self.dos_protecc = dict()
		...
		with open(l2config, 'rb') as rules:
			...
			for line in csvreader:
				...
				self.disbaled_MAC_pair.add((mac_0,mac_1))
	...
	# Marco: Add a flag to switch between dropped and normal traffic
	def installFlow(self, event, offset, srcmac, dstmac, srcip, dstip, sport,
			dport, nwproto, normal=None):
		msg = of.ofp_flow_mod()
		match = of.ofp_match()
		if(srcip != None):
		...
		# Marco: Define normal flow	
                if normal is not None:
    		    msg.actions.append(of.ofp_action_output(port = of.OFPP_NORMAL))
		event.connection.send(msg)

	def replyToIP(self, packet, match, event, fwconfig):
		# Marco: refactor code used to process data and install flows to be more generic
		def process(prio,srcmac, dstmac, s_ip, d_ip, s_port, d_port, nw_proto, normal=False):
			...
			print (prio1,s_ip1, d_ip1, s_port1, d_port1,nw_proto)
			self.installFlow(event,prio1, srcmac1, dstmac1, s_ip1, d_ip1, s_port1, d_port1, nw_proto, normal=normal)

		srcmac = EthAddr(str(match.dl_src))
		dstmac = EthAddr(str(match.dl_dst))
		sport = int(str(match.tp_src))
		dport = int(str(match.tp_dst))
		nwproto = match.nw_proto
		srcip = str(match.nw_src)
		dstip = str(match.nw_dst)

		# Marco: If packet is coming from known MAC address and unknown IP then block MAC
		# address by adding to the disabled MAC pairs (ANY dest) and calling the L2 
		# init function (previously only called on POX connect).
		if srcmac in self.dos_protecc and srcip != self.dos_protecc[srcmac]:
			if (srcmac, None) not in self.disbaled_MAC_pair:
				self._install_l2_block(event, srcmac, None)
				self.disbaled_MAC_pair.add((srcmac, None))
			return
		# Marco: Add an entry into the DOS protection map for src MAC and IP. Tracks all
		# IP traffic through the controller.
		self.dos_protecc[srcmac] = srcip
                process(2000,srcmac,dstmac,srcip,dstip,sport, dport,nwproto, normal=True)
			...
			for row in self.rules:
				...
				assert nw_proto1 is not None
				process(prio, srcmac, dstmac, s_ip, d_ip, s_port, d_port, nw_proto1)


	def _install_l2_block(self, event, src, dst):
		# Marco: refactor code from _handle_ConnectionUp to dynamically add L2 flows 
		log.debug("Blocking MAC addresses: %s ==> %s" % (src, dst))
		message = of.ofp_flow_mod() # OpenFlow massage. Instructs a switch to install a flow
		match = of.ofp_match() # Create a match
		match.dl_src = EthAddr(src) if src is not None else None # Source address
		match.dl_dst = EthAddr(dst) if dst is not None else None # Destination address
	...
	def _handle_ConnectionUp (self, event):
		...
		for (source, destination) in self.disbaled_MAC_pair:
			...
			# Marco: refactored
			self._install_l2_block(event, source, destination)
